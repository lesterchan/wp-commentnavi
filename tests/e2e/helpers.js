/**
 * Shared fixtures for the WP-CommentNavi end-to-end suite.
 *
 ...
 * row as the released 1.12.2 wrote it, a row the sanitizer has never seen, and
 ...
 *
 * The rows are read *raw* on the way back. WP_CommentNavi_Options::get() merges
 * the defaults over whatever is stored, so it answers the same array for a row
...
const VERSION_OPTION = 'wp_commentnavi_version';

/** The settings row every release up to 1.12.2 used. */
const LEGACY_OPTION = 'commentnavi_options';

/**
 * The three settings 2.0.0 added, which a 1.12.2 row cannot have held.
 *
 * Subtracted from the shipped defaults to build the legacy fixture, so the
 * fixture is derived from the running code rather than transcribed from a
 * release nobody can diff against any more.
 */
const KEYS_ADDED_IN_2 = [
	'use_commentnavi_css',
...
/**
 * The settings row a stock 1.12.2 install carried.
 *
 * Every text default has said the same thing since 1.00, so the shipped
 * defaults *are* the untouched legacy row -- minus the three settings 2.0.0
 * added, which no 1.12.2 site can have stored.
 *
...
function stockLegacyOptions( overrides = {} ) {
	...
	for ( const key of KEYS_ADDED_IN_2 ) {
		delete legacy[ key ];
	}
 ...
 * `commentnavi_options`, no version markers, and nothing else.
 *
...
/**
 * The pre-2.0.0 settings row, false once the migration has folded it in.
 *
...
function reactivatePlugin() {
	wpEval(
		...
		deactivate_plugins( 'wp-commentnavi/wp-commentnavi.php' );
		activate_plugin( 'wp-commentnavi/wp-commentnavi.php' );
		echo '<<<done>>>';`,
	);
}
...
function ensurePluginActive() {
	wpEval(
		`require_once ABSPATH . 'wp-admin/includes/plugin.php';
		if ( ! is_plugin_active( 'wp-commentnavi/wp-commentnavi.php' ) ) {
			activate_plugin( 'wp-commentnavi/wp-commentnavi.php' );
		}
		echo '<<<done>>>';`,
	);
