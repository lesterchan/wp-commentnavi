...
class Test_CommentNavi_Options extends WP_UnitTestCase {
	...
	public function test_option_key_is_unchanged() {
		$this->assertSame( 'commentnavi_options', CommentNavi_Options::OPTION_NAME );
	}
	...
	public function test_defaults_when_nothing_stored() {
		$this->assertSame( CommentNavi_Options::get_defaults(), CommentNavi_Options::get() );
	}
	...
	public function test_default_values() {
		$defaults = CommentNavi_Options::get_defaults();
	...
	public function test_larger_page_numbers_are_off_by_default() {
		$this->assertSame( 0, CommentNavi_Options::get_defaults()['num_larger_page_numbers'] );
	}
	...
	public function test_partial_row_is_merged_over_the_defaults() {
		...
		$options = CommentNavi_Options::get();

		$this->assertSame( 9, $options['num_pages'] );
		$this->assertSame( 2, $options['style'] );
		$this->assertSame( CommentNavi_Options::get_defaults()['page_text'], $options['page_text'] );
	...
	public function test_reading_a_single_key() {
		...
		$this->assertNull( CommentNavi_Options::get( 'no_such_key' ) );
	}
	...
	public function test_non_array_row_falls_back_to_defaults() {
		update_option( CommentNavi_Options::OPTION_NAME, 'not an array' );
	...
	public function test_kses_keeps_an_inline_svg() {
		...
		$filtered = CommentNavi_Options::kses( $svg );
	...
	public function test_kses_strips_scripts_and_handlers() {
		$this->assertStringNotContainsString( '<script', CommentNavi_Options::kses( '<script>alert(1)</script>' ) );
		$this->assertStringNotContainsString( 'onload', CommentNavi_Options::kses( '<svg onload="alert(1)"></svg>' ) );
	}
	...
	public function test_kses_filters_the_xlink_href_protocol() {
		$filtered = CommentNavi_Options::kses( '<svg><use xlink:href="javascript:alert(1)"></use></svg>' );
	...
	public function test_kses_does_not_leave_the_uri_list_widened() {
		CommentNavi_Options::kses( '<svg></svg>' );
	...
	public function test_kses_rejects_a_non_scalar() {
		$this->assertSame( '', CommentNavi_Options::kses( array( 'a', 'b' ) ) );
	}
}
