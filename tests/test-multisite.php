...
/**
 * WP_CommentNavi::activate() across a network.
 */
class WP_CommentNavi_Multisite_Test extends WP_CommentNavi_TestCase {
	...
	protected function seed_network( $count = 3 ) {
		...
		foreach ( $site_ids as $index => $site_id ) {
			...
			update_option( WP_CommentNavi_Options::LEGACY_OPTION, array( 'num_pages' => $num_pages ) );
	...
	public function test_network_activation_upgrades_every_site() {
		...
		foreach ( $seeded as $site_id => $num_pages ) {
			...
			$this->assertSame(
				$num_pages,
				WP_CommentNavi_Options::get( 'num_pages' ),
