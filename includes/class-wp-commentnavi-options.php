...
class WP_CommentNavi_Options {
	...
	public static function allowed_html() {
		...
		/**
		 * Filters the tags allowed inside the WP-CommentNavi navigation text options.
		 *
		 ...
		 */
		return apply_filters( 'wp_commentnavi_allowed_html', $allowed );
	}
	...
	public static function kses( $value ) {
		...
		// hand-crafted request posting commentnavi_options[prev_text][] to
		// options.php, and casting an array to string is both meaningless and a
		// PHP 8 notice.
		if ( ! is_scalar( $value ) ) {
			return '';
		}
	...
	public static function get_defaults() {
		return array(
			...
			/*
			 * Larger page numbers are new in 2.0.0 and default to off, unlike the
			 * same feature in WP-PageNavi, which has always had it and defaults to
			 * 3. Defaulting to 3 here would silently add extra links to the comment
			 * navigation of every site that upgrades, which is a visible change
			 * nobody asked for. Set it on the settings screen to turn the feature on.
			 */
	 ...
	 * up keys that did not exist when its row was written. Before 2.0.0 nothing
	 * merged, so every such key raised a notice on the front end.
	 *
	 ...
	 * `default`, which installs a `default_option_wp_commentnavi_options` filter answering with
